	...
	function(gff_file, subtracted) {
		...
		my_gd_df <-
			...
			dplyr::mutate(is_syn = dplyr::if_else(condition = (type_mut == "SNP"),
												  true = (aa_ref == aa_mut),
												  false = NA)) %>%
			dplyr::select(origin, gene, product, type_mut, is_syn, aa_ref, aa_mut, codon_ref, codon_mut, codon_pos, ref_seq, new_seq, size, codon_start, name, inference, type_ref, ID, dist_to_feat, strand_mut, start_mut, start_ref, end_ref)
